...
const ArticleCardSkeleton = ({ index }: { index: number }) => (
  <div className="group relative bg-card/80 backdrop-blur-sm border border-border/50 rounded-2xl overflow-hidden shadow-lg mb-6">
...
const InfiniteScrollSkeleton = () => (
  <div className="space-y-6">
    {Array.from({ length: 3 }).map((_, index) => (
      <ArticleCardSkeleton key={`skeleton-${index}`} index={index} />
    ))}
  </div>
);
