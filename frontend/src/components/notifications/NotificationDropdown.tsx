...
type NotificationDropdownProps = object;

const NotificationDropdown: React.FC<NotificationDropdownProps> = () => {
  const { user } = useAuth();

  // Always call useNotifications unconditionally (never in try/catch or conditionally)
  const {
    notifications = [],
    unreadCount = 0,
    isLoading = false,
              ...
              <div className="space-y-1">
                {safeNotifications.slice(0, 10).map((notification) => (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -10 }}
                    className={`relative p-3 rounded-lg border-l-4 transition-all hover:bg-muted/50 cursor-pointer ${
