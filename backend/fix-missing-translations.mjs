...
const RATE_LIMIT_CONFIG = {
  ...
  batchSize: 5, // Smaller batch size for missing translations
...
async function fixMissingTranslations() {
  logger.info('🌏 Starting missing translation fix for articles...\n');
  ...
  try {
    ...
    // Find articles that have description but missing title or content translations
    const articlesToFix = await News.find({
      ...
      $or: [
        ...
        { 'content.kh': null },
        { 'content.kh': '' },
        // Content is copied from English
        { $expr: { $eq: ['$content.kh', '$content.en'] } }
      ]
    }).sort({ createdAt: -1 });
    
    logger.info(`📊 Found ${articlesToFix.length} articles with missing translations\n`);
    
    if (articlesToFix.length === 0) {
      logger.info('🎉 All articles are properly translated!');
      return;
    }
    ...
    logger.info(`\n🎉 Missing Translation Fix Complete!`);
